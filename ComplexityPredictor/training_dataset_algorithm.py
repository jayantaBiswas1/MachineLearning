...
def input_data():
	...
	print("--------------------------------------------------------------------------------------------------")
	complex_application_check = int(input("\n Q1) Please Enter 1 if the Application used in your Use Case is in the Exception List, else please Enter 0 - "))
	number_of_gui_screens = float(input("\n Q2) Please Enter the Number of GUI screens to be traversed - "))
	number_of_manual_steps = float(input("\n Q3) Please Enter The Number of Manual Steps (Click, Read and Write) - "))
	amount_of_time = float(input("\n Q4) Enter the Time taken for each transaction in minutes (average out any pre and post activity with the number of transactions) -  "))
	type_of_application = int(input("\n Q5) Please enter the Type of Target Application Access. Enter 0 if It is Direct Application and 1 if it is Remote -  "))
	expected_volume = float(input("\n Q6) Please Enter the Expected Volume of Transactions per run (Average per run, divide by the number of transactions) -  "))
	expected_fte_workload = float(input("\n Q7) Please Enter the Expected FTE workload (1,2,3, etc.) - "))
	sla_time = int(input("\n Q8) Enter 0 if there is no strict SLA, 1 if there is SLA -  "))
	stability_of_application = int(input("\n Q9) Please Enter The Stability of the Target Applications, Enter 0 if the applications are high stability and 1 if it is Low -  "))
	business_workflows = float(input("\n Q10) Please Enter the Number of known business workflows - "))
	business_exceptions = float(input("\n Q11) Please Enter the Number of known business exceptions - "))
	integration_required = int(input("\n Q12) Does your Use Case Require NLP, ML, API, iOCR, Interact and other integration with RPA? Please Enter 1 if Yes, else Enter 0 -  "))
	complex_methods_used = int(input("\n Q13) Do you require any backend scripting? Enter 1 for yes and 0 for No - "))
	print("--------------------------------------------------------------------------------------------------------")
...
def main():
	...
	a,b,c,d,e,f,g,h,i,j,k,l,m = input_data()
	...
	if overriding_parameter_present:
		...
		if any(y == 7 for y in reason_overriding):
			...
			print("\n Overwriting Parameter for your Use Case: Scripting Needed \n Hence OverWriting Parameter Complexity ----- Medium ")
			if((y_kmeans_pred==0) or (y_kmeans_pred==1)):
				print("\n Overall Complexity for Use Case is Predicted as Medium")
				print("\n As Scripting would be required for your use case, hence total number of development days is estimated between 30-40 days or 6-8 weeks")
			else:
				print("\n Overall Complexity for your Use Case is predicated as Complex")
				print("\n The Number of Development Days assigned for your use case is close to ",weeks_predicted,"-",weeks_predicted+1," weeks or close to ",int(dev_days_predict),"-",int(dev_days_predict)+5," days")
		if flag_overriding==1:
			...
			print("\n The Number of Development Days assigned for your use case based on your process specific parameters is close to ",weeks_predicted,"-",weeks_predicted+1," weeks or close to ",int(dev_days_predict),"-",int(dev_days_predict)+5," days")
		...
		if(y_kmeans_pred == 0):
			...
			if dev_days_predict>15:
				print("\n The Number of Development Days assigned for your use case is close to 2 weeks or 10 days")
			else:
				 print("\n The Number of Development Days assigned for your use case is close to ",weeks_predicted,"-",weeks_predicted+1," weeks or close to ",int(dev_days_predict),"-",int(dev_days_predict)+5," days")
			...
			if dev_days_predict>40:
				print("\n The Number of Development Days assigned for your use case is close to 6-7 weeks or 30-35 days")
			else:
				 print("\n The Number of Development Days assigned for your use case is close to ",weeks_predicted,"-",weeks_predicted+1," weeks or close to ",int(dev_days_predict),"-",int(dev_days_predict)+5," days")
		else:
			print("---------------------------------------------------------------------------------------")
			print("\n There are No OverRding Parameters in your Use Case \n \n Overall Complexity for your Use Case is predicated as Complex \n")
			if dev_days_predict>60:
				print("\n The Number of Development Days assigned for your use case is close to 9-10 weeks or 45-50 days")
			else:
				 print("\n The Number of Development Days assigned for your use case is close to ",weeks_predicted,"-",weeks_predicted+1," weeks or close to ",int(dev_days_predict),"-",int(dev_days_predict)+5," days")
	print("\n ------------------------------------------------------------------------------------------------------------\n")
